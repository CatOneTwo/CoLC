# intermediate fusion dataset with point cloud downsample for HINTED
# HINTED: Hard Instance Enhanced Detector with Mixed-Density Feature Fusion for Sparsely-Supervised 3D Object Detection https://github.com/xmuqimingxia/HINTED
import random
import math
...
def getIntermediatedownsampleFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class IntermediateDownsampleFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            self.dataset='co'
            if self.root_dir.find('v2vreal')!=-1:
                self.dataset='v2vreal'
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            if self.load_lidar_file or self.visualize:
                ...
                # random downsample for x-axis nearest points for HINTED
                # if self.train:
                ...
                target_near_point_count = int(near_points_count / 5) # 近邻采样20%
                ...
                downsampled_lidar_np = np.concatenate((downsampled_near_point_cloud, far_points), 0)
                # x,y,z in ego space
                downsampled_projected_lidar = \
                    box_utils.project_points_by_matrix_torch(downsampled_lidar_np[:, :3],
                                                                transformation_matrix)

                if self.proj_first:
                    lidar_np[:, :3] = projected_lidar
                    downsampled_lidar_np[:, :3] = downsampled_projected_lidar

                if self.visualize:
                    # filter lidar
                    selected_cav_processed.update({'projected_lidar': projected_lidar,
                                                   'downsampled_projected_lidar': downsampled_projected_lidar})
                ...
                downsampled_processed_lidar = self.pre_processor.preprocess(downsampled_lidar_np)
                selected_cav_processed.update({'processed_features': processed_lidar,
                                               'downsampled_processed_features': downsampled_processed_lidar})

            if self.supervise_single:
                # generate targets label single GT, note the reference pose is itself.

                if self.dataset=='v2vreal':
        ...
        def __getitem__(self, idx):
            ...
            agents_image_inputs = []
            processed_features = []
            downsampled_processed_features = []
            object_stack = []
            object_id_stack = []
            single_label_list = []
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                # hys modified, v2v4real is not list
            ...
            if self.dataset=='v2vreal':
                lidar_poses=np.ones((1,6))
                lidar_poses_clean=np.ones((1,6))
            else:
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if self.load_lidar_file:
                    ...
                    downsampled_processed_features.append(selected_cav_processed['downsampled_processed_features'])
                if self.load_camera_file:
            ...
            if self.load_lidar_file:
                ...
                merged_downsampled_feature_dict = merge_features_to_dict(downsampled_processed_features)
                processed_data_dict['ego'].update({'processed_lidar': merged_feature_dict,
                                                   'downsampled_processed_lidar':merged_downsampled_feature_dict})
            if self.load_camera_file:
            ...
            if self.visualize:
                processed_data_dict['ego'].update({'origin_lidar':
                    np.vstack(
                        projected_lidar_stack)})
                processed_data_dict['ego'].update({'origin_lidar_list':
                        projected_lidar_stack})
        ...
        def collate_batch_train(self, batch):
            ...
            downsampled_processed_lidar_list = []
            image_inputs_list = []
            ...
            for i in range(len(batch)):
                ...
                if self.load_lidar_file:
                    ...
                    downsampled_processed_lidar_list.append(ego_dict['downsampled_processed_lidar'])
                if self.load_camera_file:
                ...
                if self.visualize:
                    ...
                    origin_lidar_list.append(ego_dict['origin_lidar_list'])
            ...
            if self.load_lidar_file:
                ...
                merged_downsampled_feature_dict = merge_features_to_dict(downsampled_processed_lidar_list)
                processed_lidar_torch_dict = \
                    self.pre_processor.collate_batch(merged_feature_dict)
                processed_downsampled_lidar_torch_dict = \
                    self.pre_processor.collate_batch(merged_downsampled_feature_dict)
                
                output_dict['ego'].update({'processed_lidar': processed_lidar_torch_dict,
                                           'downsampled_processed_lidar': processed_downsampled_lidar_torch_dict})
            ...
            if self.visualize:
                ...
                origin_lidar_list_torch = []
                for lidar in origin_lidar_list:
                    origin_lidar_temp = \
                        np.array(downsample_lidar_minimum(pcd_np_list=lidar))
                    origin_lidar_temp = torch.from_numpy(origin_lidar_temp)
                    origin_lidar_list_torch.append(origin_lidar_temp)

                output_dict['ego'].update({'origin_lidar_list': origin_lidar_list_torch})
    ...
    return IntermediateDownsampleFusionDataset
