    ...
    class LateFusionDataset(cls):
        ...
        def get_item_single_car(self, selected_cav_base):
            ...
            object_bbx_center, object_bbx_mask, object_ids = self.generate_object_center_single(
                [selected_cav_base], np.identity(4) if self.isv2vreal else selected_cav_base["params"]["lidar_pose_clean"]
            )
        ...
        #     """
        #     pred_box_tensor, pred_score, box_num = self.post_processor.post_process(
        #         data_dict, output_dict
        #     ) # box num is used to calculate the comm size of late fuion
        #     gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict)

        #     return pred_box_tensor, pred_score, gt_box_tensor, box_num
