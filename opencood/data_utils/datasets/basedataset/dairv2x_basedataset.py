...
# 参考：https://github.com/MediaBrain-SJTU/CoBEVFlow/blob/main/opencood/data_utils/datasets/intermediate_fusion_dataset_dair_delay.py
# 20251103 latency
def load_json(path):
...
class DAIRV2XBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        self.generate_object_center_single = self.generate_object_center_single
        ...
        if "latency_setting" not in self.params:
            self.params['latency_setting'] = OrderedDict()
            self.params['latency_setting']['add_latency'] = False
            self.params['latency_setting']['time_delay'] = 0
        self.add_latency=self.params['latency_setting']['add_latency']
        self.time_delay=self.params['latency_setting']['time_delay']

        print("ASync dataset with {} time delay initialized! {} samples totally!".format(self.time_delay, len(self.split_info)))
    ...
    def is_valid_id(self, veh_frame_id):
        ...
        if (
            int(inf_frame_id) - self.time_delay < int(cur_inf_info["batch_start_id"])
            or id_to_str(int(inf_frame_id) - self.time_delay) not in self.inf_idx2info
        ):
            return False
    ...
    def retrieve_base_data(self, idx):
        ...
        # 20251103 latency
        if self.add_latency and self.is_valid_id(veh_frame_id):
        ...
        data[0]['params']['vehicles_front'] = read_json(os.path.join(self.root_dir,frame_info['cooperative_label_path'].replace("label_world", "label_world_ori"))) 
        if self.label_sparse:
            data[0]['params']['vehicles_all'] = read_json(os.path.join(self.root_dir,frame_info['cooperative_label_path']).replace('label_world','label_world_sparse')) 
        else:
        ...
        data[0]['params']['vehicles_single_front'] = read_json(os.path.join(self.root_dir, \
                                'vehicle-side/label/lidar_ori/{}.json'.format(veh_frame_id)))
        if self.label_sparse:
            data[0]['params']['vehicles_single_all'] = read_json(os.path.join(self.root_dir, \
                                'vehicle-side/label/lidar_sparse/{}.json'.format(veh_frame_id)))
        else:
            data[0]['params']['vehicles_single_all'] = read_json(os.path.join(self.root_dir, \
        ...
        if self.label_sparse:
            data[1]['params']['vehicles_single_all'] = read_json(os.path.join(self.root_dir, \
                                'infrastructure-side/label/virtuallidar_sparse/{}.json'.format(inf_frame_id)))
        else:
            data[1]['params']['vehicles_single_all'] = read_json(os.path.join(self.root_dir, \
                                'infrastructure-side/label/virtuallidar/{}.json'.format(inf_frame_id)))
        # data[1]['params']['vehicles_single_all'] = read_json(os.path.join(self.root_dir, \
