...
class V2XSIMBaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        # hys 是否弱监督
        if self.train:
            self.label_sparse = False if 'label_sparse' not in params else params['label_sparse'] 
            self.root_dir = self.root_dir.replace('train','train_plus')
        else:
        ...
        # hys 使用全监督标签验证伪标签质量 update on 240303
    ...
    def reinitialize(self):
        ...
        for i, scene_info in enumerate(self.dataset_info_pkl):
            ...
            for j, cav_id in enumerate(cav_ids):
                ...
                current_timestamp = scene_info['timestamp']
                # 20251103 latency
                # 参考：https://github.com/IndigoChildren/collaborative-perception-MRCNet/blob/main/data_utils/datasets/basedataset/v2xsim_basedataset.py
                
                if self.add_latency and j > 0 and current_timestamp !=6:
                    # cav 第i帧的数据实际为self.time_delay前的数据

                    # the max number of hisrotical frames can be extacted
                    num_his_frames = current_timestamp - 6 
                    timestamp_delay = self.time_delay // 100
                    timestamp_delay = min(num_his_frames, timestamp_delay) # 最多能提取的历史帧数
                    timestamp_index = i
                    timestamp_index_delay = timestamp_index - timestamp_delay

                    scene_info_delay = self.dataset_info_pkl[timestamp_index_delay]
                    
                    self.scene_database[i][cav_id]['lidar'] = scene_info_delay[f'lidar_path_{cav_id}']
                    # need to delete this line is running in /GPFS
                    self.scene_database[i][cav_id]['lidar'] = \
                        self.scene_database[i][cav_id]['lidar'].replace("/home/hys/datasets/V2X-Sim", "dataset/V2X-Sim-2.0")

                    self.scene_database[i][cav_id]['params'] = OrderedDict()
                    self.scene_database[i][cav_id][
                        'params']['lidar_pose'] = tfm_to_pose(
                            scene_info_delay[f"lidar_pose_{cav_id}"]
                        )  # [x, y, z, roll, pitch, yaw]
                    self.scene_database[i][cav_id]['params'][
                        'vehicles'] = scene_info_delay[f'labels_{cav_id}'][
                            'gt_boxes_global']
                    self.scene_database[i][cav_id]['params'][
                        'object_ids'] = scene_info_delay[f'labels_{cav_id}'][
