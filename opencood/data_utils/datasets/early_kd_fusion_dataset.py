...
def getEarlykdFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class EarlyKDFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            # ERMVP: Communication-Efﬁcient and Collaboration-Robust Multi-Vehicle Perception in Challenging Environments (2024 CVPR)
            ...
            if self.cav_lidar_sample:
                ...
                self.nonei = False if 'nonei' not in params['fusion']['args'] else params['fusion']['args']['nonei']
            ...
            if self.kd_flag:
                self.singlestudent = params['kd_flag']['singlestudent'] # 学生是否只有一个智能体
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            if self.load_lidar_file or self.visualize:
                ...
                if self.cav_lidar_sample:
                    if self.cav_lidar_sample['method'] == 'RS': # random sampling, 均匀随机采样
                    ...
                    elif self.cav_lidar_sample['method'] == 'FAPS':
                        #  self.proj_first=True 必须的！
                        ...
                        lidar_np_bg = lidar_np[~fg_mask]
                        bg_points_count = lidar_np_bg.shape[0]
                        target_point_count = int(bg_points_count* self.cav_lidar_sample['bg_ratio']) # 随机采样10%
                        random_indices = np.random.choice(bg_points_count, target_point_count, replace=False)
                        lidar_np_ds = lidar_np_bg[random_indices]

                        if fg_mask.sum() > 0:
                            lidar_np_fg = lidar_np[fg_mask]
                            fps_idx = fps_cluster(torch.tensor(lidar_np_fg), ratio=self.cav_lidar_sample['fg_ratio'])
                            lidar_np_fg_ds = lidar_np_fg[fps_idx.numpy()]
                            lidar_np_ds = np.concatenate((lidar_np_fg_ds, lidar_np_ds))
        ...
        def __getitem__(self, idx):
            ...
            # print([ego_id == cav_id for cav_id in cav_id_list]) # in current batch, the first agent is ego


            # if self.isdairv2x and cav_num > 1:
            #     print(cav_id_list)
            #     cav_id_list=[1] # vqvae只训练infra        
            ...
            if self.load_lidar_file:
                ...
                if self.cav_lidar_sample:
                    if len(processed_features_downsample) > 0:
