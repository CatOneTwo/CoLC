...
def train_parser():
    ...
    # parser.add_argument('--noise_setting', type=str, default=True,
    #                     help='if add noise setting?')
    parser.add_argument('--compression_setting', type=str, default=False,
                        help='Whether to compress features?')
    parser.add_argument('--pretrained_model', type=str, default='',
                        help='load pretrained model')
    parser.add_argument('--run_test', dest='run_test', action='store_true',
                        help='Run inference after training (default)')
    parser.add_argument('--no-run_test', dest='run_test', action='store_false',
...
def main():
    ...
    if 'cav_lidar_sample' in hypes['fusion']['args']:
        ds_info = hypes['fusion']['args']['cav_lidar_sample']['method']+'_'+ str(hypes['fusion']['args']['cav_lidar_sample']['ratio'])
        if opt.log == '':
            opt.log = ds_info
        else:
            opt.log +=  '_' + ds_info 
    ...
    #     if opt.noise_setting:
    #         hypes['noise_setting']['add_noise']=True
    #     else:
    #         hypes['noise_setting']['add_noise']=False

    if opt.compression_setting:
        hypes['model']['args']['compression']=32
        hypes['noise_setting']['add_noise']=False
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    # final_loss = criterion(ouput_dict, batch_data['ego']['label_dict'])
                    final_loss = criterion(ouput_dict, batch_data['ego']['label_dict'], validate=True)
    ...
    if opt.run_test:
        ...
        if 'noise_setting' in hypes and hypes['noise_setting']['add_noise']:
            cmd = f"CUDA_VISIBLE_DEVICES=0 python opencood/tools/inference_w_noise.py --model_dir {saved_path} --fusion_method {fusion_method}"
        else:
            cmd = f"CUDA_VISIBLE_DEVICES=0 python opencood/tools/inference.py --model_dir {saved_path} --fusion_method {fusion_method}"
