...
def load_pretrained_model(saved_path, model):
    ...
    if file_list:
        ...
        print("load best validation pretrained model at epoch %d" % \
    ...
    if initial_epoch > 0:
        print('load pretrained model at epoch %d' % initial_epoch)
...
def load_saved_model(saved_path, model, initial_epoch=-1):
    """
...
def setup_train(hypes):
    ...
    suffix= hypes['log_suffix']
    if suffix!='':
        suffix = '_'+suffix

    if model_name.find('_withnoise') != -1:
        model_name = model_name.replace('_withnoise','')
