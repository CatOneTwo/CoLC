...
from opencood.models.fuse_modules.att_fuse import ScaledDotProductAttention
from opencood.models.comm_modules.where2comm import Communication4cosdh as Communication
...
class CoBEVT(nn.Module):
    ...
    def forward(self, x, record_len, affine_matrix):
        _, C, H, W = x.shape # x: [sum(record_len), C, H, W]
        B, L = affine_matrix.shape[:2] # affine_matrix [B, L, L, 2, 3]
        regroup_feature, mask = Regroup(x, record_len, L) # regroup_feature [B, L, C, H, W]  mask:[B,L] 记录每个batch的agent [1,0]
        com_mask = mask.unsqueeze(1).unsqueeze(2).unsqueeze(3) # [B, 1, 1, 1, L]
        com_mask = repeat(com_mask,
                          'b h w c l -> b (h new_h) (w new_w) c l',
                          new_h=regroup_feature.shape[3],
                          new_w=regroup_feature.shape[4]) # [B, H, W, 1, L]

        # 坐标转换
        regroup_feature_new = []
        for b in range(B):
            ego = 0
            regroup_feature_new.append(warp_affine_simple(regroup_feature[b], affine_matrix[b, ego], (H, W))) # [L, C, H, W]
...
class Where2comm(nn.Module):
    ...
    def __init__(self, args, dim):
        ...
        if args['fusion'] == 'att':
            ...
            self.fuse_modules = MaxFusion4cosdh()
    ...
    def forward(self, x, psm_single, record_len, normalized_affine_matrix, req_mask=None):
        ...
        x_out = self.fuse_modules(warp_x, record_len, normalized_affine_matrix)
        return x_out, communication_rates


class MaxFusion4cosdh(nn.Module):
    def __init__(self):
        super(MaxFusion4cosdh, self).__init__()
