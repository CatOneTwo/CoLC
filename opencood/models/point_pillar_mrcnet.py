...
class PointPillarMRCNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.use_temporal:
            ...
            if 'dir_args' in args.keys():
                self.use_dir = True
                self.dir_head = nn.Conv2d(128, args['dir_args']['num_bins'] * args['anchor_number'], kernel_size=1) # BIN_NUM = 2
        else:
            ...
            if 'dir_args' in args.keys():
                self.use_dir = True
                self.dir_head = nn.Conv2d(128*3, args['dir_args']['num_bins'] * args['anchor_number'], kernel_size=1) # BIN_NUM = 2
    ...
    def forward(self, data_dict):
        ...
        if self.use_dir:
            output_dict.update({'dir_preds': self.dir_head(target_feature)})
