...
class SecondSSFA(nn.Module):
    ...
    def forward_features(self, data_dict):
        ...
        voxel_features = data_dict['nei_processed_lidar']['voxel_features']
        voxel_coords = data_dict['nei_processed_lidar']['voxel_coords']
        voxel_num_points = data_dict['nei_processed_lidar']['voxel_num_points']
        batch_size = voxel_coords[:,0].max() + 1 # batch size is padded in the first idx
