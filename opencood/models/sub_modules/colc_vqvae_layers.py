...
class VectorQuantizer(nn.Module):        
    def __init__(self, model_cfg):
        ...
        self.e_dim = model_cfg['e_dim']
        self.beta = model_cfg['beta']
        self.cosine_similarity = model_cfg['cosine_similarity'] if 'cosine_similarity' in model_cfg else False
        self.dead_limit = model_cfg['dead_limit'] if 'dead_limit' in model_cfg else 256
    ...
    def train_codebook(self, z, code_age, code_usage):
        ...
        if self.cosine_similarity:
            ...
            z_dist = torch.cdist(z_flattened, self.embedding.weight) # [6300, 512]
            min_encoding_indices = torch.argmin(z_dist, dim=1) # [6300]
            

        z_q = self.embedding(min_encoding_indices).view(z.shape) # [1, 6300, 512]
        ...
        if code_age is not None and code_usage is not None and self.embedding.weight.requires_grad:
...
class VQEncoder(nn.Module):
    def __init__(self, model_cfg):
        ...
        img_size = model_cfg['img_size'] if 'img_size' in model_cfg else 640
        patch_size = model_cfg['patch_size'] if 'patch_size' in model_cfg else 8
        in_chans = model_cfg['in_chans'] if 'in_chans' in model_cfg else 40
        embed_dim = model_cfg['embed_dim'] if 'embed_dim' in model_cfg else 512
        num_heads = model_cfg['num_heads'] if 'num_heads' in model_cfg else 16
        depth = model_cfg['depth'] if 'depth' in model_cfg else 12
        codebook_dim = model_cfg['codebook_dim'] if 'codebook_dim' in model_cfg else 1024
        window_size = model_cfg['window_size'] if 'window_size' in model_cfg else 8
    ...
    def forward(self, x):
        ...
        # embed patches
        x = self.patch_embed(x) # [1, 6300, 256]

        # add pos embed w/o cls token
        x = x + self.pos_embed # [1, 6300, 256]

        # apply Transformer blocks
        x = self.blocks(x) # [1, 6300, 256]
        x = self.norm(x) # [1, 6300, 256]
        x = self.pre_quant(x) # [1, 6300, 512]
...
class VQDecoder(nn.Module):
    def __init__(self, model_cfg):
        ...
        img_size = model_cfg['img_size'] if 'img_size' in model_cfg else 640
        num_patches = model_cfg['num_patches'] if 'num_patches' in model_cfg else 6400 # (img_size[0]/patch_size)* (img_size[1]/patch_size)
        patch_size = model_cfg['patch_size'] if 'patch_size' in model_cfg else 8
        in_chans = model_cfg['in_chans'] if 'in_chans' in model_cfg else 40
        embed_dim = model_cfg['embed_dim'] if 'embed_dim' in model_cfg else 512
        num_heads = model_cfg['num_heads'] if 'num_heads' in model_cfg else 16
        ...
        self.density_head= True if 'density_head' in model_cfg else False
        ...
        self.occ_pred = nn.Sequential(nn.Conv2d(in_chans, 1, kernel_size=1))
