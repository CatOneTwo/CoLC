...
def downsample_lidar_with_label(pcd_np, label_np, num):
    """
    Downsample lidar points and corresponding labels to a certain number.

    Parameters
    ----------
    pcd_np : np.ndarray
        Lidar points, shape (N, 4)
    label_np : np.ndarray
        Point labels, shape (N,)
    num : int
        Downsample target number

    Returns
    -------
    pcd_np_down : np.ndarray
        Downsampled points, shape (num, 4)
    label_np_down : np.ndarray
        Downsampled labels, shape (num,)
    """
    assert pcd_np.shape[0] >= num, "num should be <= number of points"

    selected_idx = np.random.choice(pcd_np.shape[0], num, replace=False)
    pcd_np_down = pcd_np[selected_idx]
    label_np_down = label_np[selected_idx]

    return pcd_np_down, label_np_down


def downsample_lidar_minimum_with_label(pcd_np_list, label_list):
    """
    Downsample all point clouds in a batch to the minimum number of points
    and simultaneously downsample labels.

    Parameters
    ----------
    pcd_np_list : list of np.ndarray
        Each element shape (N_i, 4)
    label_list : list of np.ndarray
        Each element shape (N_i,)

    Returns
    -------
    pcd_np_list_down : list of np.ndarray
        Downsampled point clouds, shape (min_N, 4)
    label_list_down : list of np.ndarray
        Downsampled labels, shape (min_N,)
    """
    # 找到 batch 中最小点数
    min_num = min([pcd.shape[0] for pcd in pcd_np_list])

    pcd_np_list_down = []
    label_list_down = []

    for pcd, label in zip(pcd_np_list, label_list):
        pcd_down, label_down = downsample_lidar_with_label(pcd, label, min_num)
        pcd_np_list_down.append(pcd_down)
        label_list_down.append(label_down)

    return pcd_np_list_down, label_list_down