    ...
    if proj_first:
        ...
        for cav_id, cav_content in base_data_dict.items():
            ...
            if isinstance(lidar_pose,list):
                ...
                t_list.append(lidar_pose) # v2v4real 的 lidar pose就在world坐标系下（4，4）
...
def x1_to_x2(x1, x2):
    ...
    # object pose is list while lidar pose is transformation matrix
        ...
        world_to_x2 = x2
        transformation_matrix = np.dot(world_to_x2, x1_to_world)
    # both are numpy matrix v2v4real
    else:
        world_to_x2 = np.linalg.inv(x2) # [4,4]
