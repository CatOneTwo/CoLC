...
class PointPillarLoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict, suffix="", validate=False):
        ...
        if 'out_contrast_loss' in output_dict:
            contrastive_loss = output_dict['out_contrast_loss']
            # lambda_con = output_dict['lambda_con']
            total_loss += contrastive_loss
            self.loss_dict.update({'cont_loss': contrastive_loss.item()})
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", pbar=None):
        ...
        if 'cont_loss' in self.loss_dict:
            cont_loss = self.loss_dict.get('cont_loss', 0)
            msg += " || Cont Loss: %.4f" % cont_loss
        ...
        if not writer is None:
            ...
            if 'cont_loss' in self.loss_dict:
                writer.add_scalar('Cont_loss'+suffix, cont_loss, epoch*batch_len + batch_id)
